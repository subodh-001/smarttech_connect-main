...
const Header = ({ user: userProp = null, initialLocation = null, activeService = null }) => {
  ...
  if (role === 'technician') {
    navigationItems = [
      { label: 'Overview', path: '/technician-dashboard', icon: 'Home' },
      { label: 'Job Requests', path: '/technician-dashboard?tab=jobs', icon: 'Briefcase' },
      { label: 'Active Jobs', path: '/technician-dashboard?tab=active', icon: 'Play' },
      { label: 'Earnings', path: '/technician-dashboard?tab=earnings', icon: 'DollarSign' },
      { label: 'Schedule', path: '/technician-dashboard?tab=schedule', icon: 'Calendar' },
                  ...
                  <div className="py-1">
                    {role === 'technician' ? (
                      <>
                        ...
                        <Link
                          to="/technician-dashboard?tab=earnings"
                          className="flex items-center space-x-2 px-3 py-2 text-sm text-foreground hover:bg-muted trust-transition"
                          onClick={() => setIsProfileDropdownOpen(false)}
                        >
                        ...
                        <Link
                          to="/profile-management?tab=documents"
                          className="flex items-center space-x-2 px-3 py-2 text-sm text-foreground hover:bg-muted trust-transition"
                          onClick={() => setIsProfileDropdownOpen(false)}
                        >
